...
BEGIN
    ...
    IF productoExistencias >= _Cantidad THEN
        -- Registrar la venta en Detalle_Factura
        INSERT INTO Detalle_Factura (Cantidad, ID_Factura, ID_Producto) VALUES (_Cantidad, _ID_Factura, _ID_Producto);
...
CREATE PROCEDURE Anular_Factura(
	...
	BEGIN
		...
		-- Actualizar existencias y precio total en Productos
		UPDATE Productos SET Existencias = Existencias + _Cantidad, Precio_Total = Precio_Total + Total_Prod WHERE ID = _ID_Producto;
...
SELECT COUNT(ID) AS 'Cant. Fact Anuladas' FROM Facturas WHERE Estado = 'Anulada';
...
-- Procedimiento para buscar por nombre o por rango de fecha.
DELIMITER //
...
CREATE VIEW Mostrar_Devoluciones AS SELECT
    ...
    CONCAT('C$ ', FORMAT(SUM(c.Precio_Venta * b.Cantidad), 2)) AS 'Total',
    e.Nombre AS 'Cliente',
